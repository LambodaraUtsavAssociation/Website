  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      {label && variant !== 'iconOnly' && (
        <label
          className={`block text-[10px] uppercase tracking-[0.2em] font-semibold mb-1.5 flex items-center space-x-1.5 ${lightMode ? 'text-orange-700 font-bold' : 'text-gold-400'}`}
        >
          {icon || (
            <Filter className={`w-3 h-3 ${lightMode ? 'text-orange-500' : 'text-gold-400'}`} />
          )}
          <span>{label}</span>
        </label>
      )}
      ...
      {variant === 'iconOnly' ? (
        ...
        >
          {icon || (
            <Filter
              className={`w-5 h-5 group-hover:scale-110 transition-transform ${lightMode ? 'text-orange-500' : 'text-gold-400'}`}
            />
          )}
          {value !== 'all' && (
            <span
              className={`absolute top-1 right-1 w-2.5 h-2.5 rounded-full animate-pulse ${lightMode ? 'bg-orange-500 ring-2 ring-white' : 'bg-gold-400 ring-2 ring-charcoal-950'}`}
            />
          )}
        </button>
      ) : (
        ...
        >
          <div className="flex items-center space-x-2.5 min-w-0 pr-2">
            <div
              className={`p-1.5 rounded-lg flex-shrink-0 group-hover:scale-105 transition-transform ${lightMode ? 'bg-orange-100 text-orange-600' : 'bg-gold-500/10 border border-gold-500/20 text-gold-400'}`}
            >
              {icon || (
                <Filter
                  className={`w-3.5 h-3.5 ${lightMode ? 'text-orange-500' : 'text-gold-400'}`}
                />
              )}
            </div>
              ...
              {selectedOption ? (
                <span className="flex items-center space-x-1.5 truncate">
                  {selectedOption.sublabel && (
                    <span
                      className={`font-mono text-[11px] flex-shrink-0 ${lightMode ? 'text-orange-600 font-bold' : 'text-gold-400/80'}`}
                    >
                      {selectedOption.sublabel}
                    </span>
                  )}
                  ...
                  {selectedOption.count !== undefined && (
                    <span
                      className={`text-[10px] font-mono flex-shrink-0 ml-1 ${lightMode ? 'text-orange-600 font-bold' : 'text-gold-400 opacity-80'}`}
                    >
      ...
      {isOpen && (
        <div
          className={`absolute top-full right-0 mt-2 min-w-[260px] w-64 sm:w-72 z-[100] rounded-2xl shadow-2xl py-2 max-h-80 overflow-y-auto no-scrollbar animate-fade-in backdrop-blur-2xl ${
            lightMode
        ...
        >
          <div
            className={`px-4 py-2 border-b text-[10px] font-bold uppercase tracking-[0.2em] flex items-center justify-between ${
            ...
            return (
              <button
                ...
                className={`w-full px-4 py-2 text-xs font-bold uppercase tracking-wider text-left flex items-center justify-between transition-colors ${
                  ...
                  isHeader
                    ? 'bg-orange-50/80 font-extrabold text-orange-900 border-t border-orange-100 mt-1'
                    : ''
                } ${
                  isSelected
                    ? lightMode
