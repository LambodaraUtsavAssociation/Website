...
export default function AdminMobileBottomBar({ onOpenUpload }: AdminMobileBottomBarProps) {
  ...
  return (
    <>
      ...
      <nav aria-label="Mobile Admin Navigation" className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-xl border-t border-orange-200 shadow-2xl px-1 pt-1.5 pb-[max(0.375rem,env(safe-area-inset-bottom))] flex items-center justify-around touch-manipulation">
        {/* Home / Overview Tab */}
        <Link
          href="/admin"
        ...
        >
          ...
          <span className="text-[9px] mt-0.5 uppercase tracking-wider font-sans truncate">Overview</span>
        </Link>
        ...
        >
          <Images className={`w-4 h-4 sm:w-5 sm:h-5 ${pathname.startsWith('/admin/memories') ? 'text-orange-600' : 'text-slate-400'}`} />
        ...
        >
          <Upload className={`w-4 h-4 sm:w-5 sm:h-5 ${pathname === '/admin/upload' ? 'text-orange-600' : 'text-slate-400'}`} />
          <span className="text-[9px] mt-0.5 uppercase tracking-wider font-sans truncate">Upload</span>
        </Link>
